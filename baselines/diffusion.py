...
class IndependentCascade(object):
    def __init__(self, graph):
        ...
        self.prob_matrix = [self.graph.edges[self.reverse_edge_idx[i][0], self.reverse_edge_idx[i][1]]['weight'] for i in sorted(self.reverse_edge_idx.keys())]
    
    def sample_live_graph_mc(self, mc):
        edge_probs = {(u, v): d['weight'] for u, v, d in self.graph.edges().data()}
    ...
    def diffusion_iter(self):
        for node in self.graph.nodes:
            if self.graph.nodes[node]['is_active']:
                continue
            neighbors = self.neighborhood_fn(node)
            weights = sum(self.graph.edges[node, neighbor]['weight'] for neighbor in neighbors)
